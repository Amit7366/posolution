      ...
      <div className="relative mx-auto w-full max-w-[1600px] px-6 py-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">{t("dash.stock.subtitle")}</p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 dark:border-gray-700 px-5 py-4">
            <div className="relative w-full max-w-xs">
              ...
              <input
                ...
                className="w-full rounded-xl border border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-800 py-2.5 pl-10 pr-3 text-sm text-gray-900 dark:text-slate-100 placeholder:text-gray-400 dark:placeholder:text-slate-500 outline-none focus:border-orange-500/30 focus:ring-4 focus:ring-orange-500/20"
              />
            </div>
          ...
          {(isLoading || isFetching) && !listPayload ? (
            <div className="px-5 py-10 text-center text-sm text-gray-500 dark:text-slate-400">{t("dash.common.loading")}</div>
          ) : null}
            ...
            <>
              <div className="w-full overflow-x-auto">
                <table className="w-full min-w-[1000px] text-sm">
                  <thead>
                    <tr className="text-left text-gray-600 dark:text-slate-300 border-b border-gray-200 dark:border-gray-700">
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colWarehouse")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colStore")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colProduct")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colSku")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colCategory")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colQty")}</th>
                      <th className="px-5 py-4 font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colUpdated")}</th>
                      <th className="px-5 py-4 text-right font-semibold text-gray-900 dark:text-slate-100">{t("dash.stock.colAction")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/10">
                    {rows.map((r) => (
                      <tr key={r.id} className="hover:bg-gray-50 dark:bg-white/[0.03]">
                        <td className="px-5 py-4 text-gray-600 dark:text-slate-300">{r.warehouse}</td>
                        <td className="px-5 py-4 text-gray-500 dark:text-slate-400">{r.store}</td>
                        ...
                        <td className="px-5 py-4 text-gray-500 dark:text-slate-400 font-mono text-xs">{r.sku}</td>
                        <td className="px-5 py-4 text-gray-500 dark:text-slate-400">{r.category}</td>
                        <td className="px-5 py-4 font-semibold text-orange-300">{r.quantity}</td>
                        <td className="px-5 py-4 text-gray-500 dark:text-slate-500 text-xs">{r.updatedLabel}</td>
                        <td className="px-5 py-4 text-right">
                          <button
                            type="button"
                            title={t("dash.stock.updateQty")}
                            onClick={() => setEditing(r)}
                            className="inline-grid h-9 w-9 place-items-center rounded-lg border border-gray-300 bg-gray-100 text-gray-700 hover:bg-gray-200 dark:border-gray-600 dark:bg-black/20 dark:text-white dark:hover:bg-white/[0.06]"
                          >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 dark:border-gray-700 px-5 py-4 text-sm text-gray-500 dark:text-slate-400">
  ...
  return (
    <div className="relative">
      <select
        ...
        className="h-10 rounded-lg border border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-800 px-3 pr-9 text-sm font-medium text-gray-900 dark:text-slate-100 outline-none focus:border-orange-500/30 focus:ring-4 focus:ring-orange-500/20"
        aria-label={label}
      >
        {options.map((o) => (
...
function TopIconButton({
  ...
  return (
    <button
      ...
      title={title}
      onClick={onClick}
      className="grid h-10 w-10 place-items-center rounded-lg border border-gray-300 bg-white text-gray-700 transition hover:bg-gray-100 dark:border-gray-600 dark:bg-white/[0.03] dark:text-slate-200 dark:hover:bg-white/[0.06]"
    >
      {children}
    </button>
  );
...
function PageNavButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "grid h-8 w-8 place-items-center rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/[0.02] text-gray-700 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-white/[0.06]",
