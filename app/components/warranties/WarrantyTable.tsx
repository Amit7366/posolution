      ...
      <table className="w-full min-w-[1200px]">
        <thead>
          <tr className="text-left text-sm text-gray-600 dark:text-slate-300">
            ...
            <th className="px-5 py-4 font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{t("dash.warranties.colWarranty")}</th>
            <th className="px-5 py-4 font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{t("dash.warranties.colDesc")}</th>
            <th className="px-5 py-4 font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{t("dash.common.duration")}</th>
            <th className="px-5 py-4 font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{t("dash.common.status")}</th>
            <th className="w-52 px-5 py-4 text-right font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{t("dash.common.actions")}</th>
          </tr>
        </thead>

        <tbody className="divide-y divide-white/10">
          {rows.map((r) => (
            <tr key={r.id} className="group hover:bg-gray-50 dark:bg-white/[0.03]">
              ...
              <td className="px-5 py-4 text-sm font-semibold text-gray-900 dark:text-gray-900 dark:text-slate-100">{r.name}</td>

              <td className="px-5 py-4 text-sm text-gray-600 dark:text-slate-300">
                <div className="max-w-[680px] truncate">{r.description}</div>
              </td>

              <td className="px-5 py-4 text-sm text-gray-500 dark:text-slate-400">{warrantyLabel(r)}</td>
          ...
          {rows.length === 0 && (
            <tr>
              <td colSpan={6} className="px-5 py-14 text-center text-sm text-gray-500 dark:text-slate-400">
