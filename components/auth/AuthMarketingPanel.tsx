...
export default function AuthMarketingPanel() {
  ...
  return (
    <div className="relative flex h-full min-h-[280px] flex-col overflow-hidden bg-linear-to-b from-brand-blue via-brand-blue-deep to-brand-navy text-white transition-colors duration-300 dark:from-brand-navy dark:to-neutral-950 lg:min-h-0">
      <div
        aria-hidden
        className="pointer-events-none absolute -left-20 top-1/4 h-72 w-72 rounded-full bg-brand-cyan/30 blur-3xl dark:bg-brand-cyan/20"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-16 bottom-1/3 h-64 w-64 rounded-full bg-brand-green/20 blur-3xl dark:bg-brand-emerald/15"
      />

      <div className="relative z-10 flex items-center justify-between px-6 pt-6 sm:px-8 lg:px-10">
        <BrandLogo size="md" onDark wordmark={t("auth.marketing.brand")} />
      </div>
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center px-6 py-8 lg:py-4">
        <div className="relative h-48 w-48 sm:h-56 sm:w-56 lg:h-64 lg:w-64">
          ...
          <motion.div
            className="absolute inset-[12%] rounded-full bg-linear-to-br from-brand-cyan/90 to-brand-blue/80 shadow-[0_0_60px_rgba(1,167,188,0.35)]"
